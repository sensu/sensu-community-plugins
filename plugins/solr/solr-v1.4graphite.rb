  ...
  option :scheme,
	...
	def lookingfor (needle, haystack)
		haystack.each_with_index do |element, index|
			if element.css('name').text.strip == needle; then return index else next end
		end
	end
	...
	def run
		...
		#Capture initial stats page XML data.
		doc = Nokogiri::XML(open("http://#{config[:host]}:#{config[:port]}/solr/admin/stats.jsp"))
		...
		doc.css('CORE entry').each do |coreinfo|
			...
			output [config[:scheme], 'CORE', coreinfo.css('name').text.strip, 'warmupTime'].join("."), coreinfo.css("stats stat[name='warmupTime']").text.strip, Time.now.to_i
		end
		...
		statcache			 = Array['numDocs', 'maxDoc', 'warmupTime']
