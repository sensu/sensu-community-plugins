  ...
  def run
    ...
    if config[:queuelevel]
      ...
      rabbitmq.queues.each do |queue|
        ...
        (crit_queues["#{queue['name']}"] = queue['messages']; next) if queue['messages'] >= config[:critical].to_i # rubocop: disable Semicolon
        (warn_queues["#{queue['name']}"] = queue['messages']; next) if queue['messages'] >= config[:warn].to_i # rubocop: disable Semicolon
      end
